...
**注意**：由於近期（2020下半年起）時間所限，Easer的維護速度會受到影響。不過由於程序主要框架已經穩定，除了增加對Event和Condition的遠程插件的支持（以及長期計劃中對多機器自動化互聯的支持）外，Easer的核心部分應當不會進行顯著修改。有興趣者可以查看下面的相關章節，理解如何爲Easer添加你需要的功能，或者如何定位錯誤。作者會儘量抽時間看PR，然後先發佈的beta版本測試，隨後發佈正式版本。
